...
import {
  PaymentElement,
  // AddressElement, // Commenté pour l'instant, on utilise nos champs manuels
  useStripe,
  useElements
} from '@stripe/react-stripe-js';
import { loadStripe } from '@stripe/stripe-js';
import { Elements } from '@stripe/react-stripe-js';
import { FaPhoneAlt, FaMapMarkerAlt, FaSearch } from 'react-icons/fa'; // Ajout des icônes
...
function CheckoutForm() {
  ...
  const getPackageSize = (itemCount) => {
  ...
  // Réinitialiser les informations pré-remplies
  ...
  // Restaurer les informations pré-remplies
  ...
  const handleInputChange = (e) => {
    ...
    // Formater le téléphone
      ...
      // 1. Nettoyer pour ne garder que les chiffres
      const digits = value.replace(/[^0-9]/g, '');
      // 2. Limiter à 10 chiffres (pour un mobile standard FR)
      const limitedDigits = digits.slice(0, 10);
      // 3. Ajouter les espaces tous les 2 chiffres
      let formattedValue = '';
      for (let i = 0; i < limitedDigits.length; i++) {
        formattedValue += limitedDigits[i];
        // Ajouter un espace après chaque groupe de 2 chiffres, sauf à la fin
  ...
  const handleSubmitOrder = async (e) => {
    ...
    // Vérifier que l'élément de paiement est bien monté
    ...
    // Mettre à jour le log avec les nouveaux champs
    console.log("Données client pour commande :", {
        ...formData,
        userId: user?._id,
    ...
    try {
      ...
      if (error) {
        ...
        console.log("Paiement réussi immédiatement:", paymentIntent);
        navigate('/order-confirmation?payment_intent_status=succeeded');
      } else if (paymentIntent) {
        console.log("Statut du paiement après confirmation:", paymentIntent.status);
      }
    } catch (err) {
  ...
  // Le JSX du formulaire est mis à jour
  return (
      ...
      <div className="container mx-auto px-6">
        ...
        <div className="grid md:grid-cols-2 gap-12">
          ...
          >
            ...
            {/* Informations de calcul des frais de port */}
            <ShippingInfo 
              shippingCalculation={shippingCalculation}
              isCalculating={isCalculatingShipping}
            />
            
            {/* Boutons de gestion des informations pré-remplies */}
            ...
            <form onSubmit={handleSubmitOrder} className="space-y-5">
              ...
              <div>
                <label htmlFor="address" className="block text-sm font-medium text-gray-700 mb-1">
                  ...
                  {user?.address && <span className="ml-2 text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded-full">Pré-rempli</span>}
                </label>
                <AddressAutocomplete
                  value={formData.address}
                  onChange={(newAddress) => {
                    setFormData(prev => ({ ...prev, address: newAddress }));
                    // Déclencher le calcul des frais de port si l'adresse est valide
                  ...
                  onSelect={(suggestion) => {
                    ...
                    // Optionnel : utiliser les coordonnées pour un calcul plus précis
...
function CheckoutPage() {
  ...
  useEffect(() => {
    if (cartTotal > 0 && isAuthenticated) {
      ...
      .then((res) => {
          ...
          if (res.status === 401) {
            console.log('Token expiré lors de la création du payment intent, déconnexion...');
            logout();
  ...
  }, [cartTotal, isAuthenticated, token, API_URL]);
